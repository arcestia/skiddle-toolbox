...
export const base64View = (): string => layout({
  ...
  body: `
    ...
    <div class="tb-card">
      <div class="b64-mode-tabs" role="tablist">
        <button type="button" class="b64-tab b64-tab--active" role="tab" aria-selected="true" data-mode="encode" onclick="b64SetMode('encode')">Encode</button>
        <button type="button" class="b64-tab" role="tab" aria-selected="false" data-mode="decode" onclick="b64SetMode('decode')">Decode</button>
      </div>
      ...
      <div class="b64-grid">
        <div class="b64-col">
          ...
          <div class="b64-file-row">
            <label class="tb-btn tb-btn-secondary b64-file-btn">
              ...
              Upload file
              <input type="file" id="b64-file" class="tb-sr-only" onchange="b64HandleFile(this)">
            </label>
            <span id="b64-file-name" class="tb-muted" style="font-size:0.8rem;"></span>
          </div>
        </div>
        <div class="b64-col">
          <label for="b64-output" class="tb-label">Output</label>
          <textarea id="b64-output" class="tb-textarea b64-textarea" readonly spellcheck="false"></textarea>
          <div class="b64-actions">
            <button type="button" class="tb-btn tb-btn-secondary" onclick="b64Copy()">
            ...
            <button type="button" class="tb-btn tb-btn-secondary" onclick="b64Clear()">Clear</button>
          </div>
        </div>
      </div>
    ...
    <style>
      ...
      .b64-file-btn { display: inline-flex; align-items: center; gap: 6px; font-size: 0.8rem; padding: 5px 12px; }
    ...
    (function() {
      ...
      window.b64SetMode = function(m) {
        mode = m;
        document.querySelectorAll('.b64-tab').forEach(t => {
      ...
      window.b64Convert = function() {
        ...
        const dataUri = document.getElementById('b64-data-uri').checked;
        const info = document.getElementById('b64-info');
        ...
        try {
          if (mode === 'encode') {
            ...
            if (toDecode.startsWith('data:')) toDecode = toDecode.split(',')[1] || '';
      ...
      window.b64HandleFile = function(el) {
        ...
        reader.onload = function() {
          if (mode === 'encode') {
            ...
            document.getElementById('b64-input').value = reader.result.split(',')[1] || '';
            b64Convert();
          }
        };
      ...
      window.b64Copy = function() {
        const text = document.getElementById('b64-output').value;
        if (!text) return;
